...
for ind in range(200):
    ...
    for i, feature in enumerate(features):
        ...
        grad = jittor.grad(feature[feature.shape[0]//2-1][feature.shape[1]//2-1], input)
        grad = jittor.abs(grad)
        grad = grad.mean(dim=1,keepdim=False).squeeze()
        heatmaps[i] = heatmaps[i] + grad.numpy()
...
plt.margins(0, 0)
plt.savefig(f'visualization/jittor_erf.png', bbox_inches = 'tight')